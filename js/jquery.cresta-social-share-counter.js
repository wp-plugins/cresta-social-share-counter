	...
	$(document).ready(function() {
		...
		totalShares($URL);
			...
			function ReplaceNumberWithCommas(shareNumber) {
				var components = shareNumber.toString().split(".");
			...
			function twitterShares($URL) {
				...
				if ( $('#twitter-cresta').hasClass('twitter-cresta-share') ) {
					...
					$.getJSON('http://urls.api.twitter.com/1/urls/count.json?url=' + $URL + '&callback=?', function (twitdata) {
			...
			function checkJSON_getSum() {
					...
					$('#total-count').text( ReplaceNumberWithCommas(totalShares) )

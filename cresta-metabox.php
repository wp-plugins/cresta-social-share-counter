...
function cresta_add_meta_box() {
	...
	foreach ( $screens as $screen ) {
		...
		add_meta_box(
			'cresta_sectionid',
			__( 'Cresta Social Share Counter', 'crestassc' ),
...
function cresta_meta_box_callback( $post ) {
	...
	<label for="cresta_new_field">
        <input type="checkbox" name="cresta_new_field" id="cresta_new_field" value="1" <?php checked( $crestaValue, '1' ); ?> /><?php _e( 'Hide icons in this page?', 'crestassc' )?>
    </label>
	<?php
}

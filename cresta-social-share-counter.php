...
function register_social_button_setting()
{
	...
	if (get_option( 'cresta_social_shares_show_credit' ) === false ) {
		add_option( 'cresta_social_shares_show_credit', '1' );
	}
...
if($show_credit == 1) {
	echo '<div class="sbutton crestaCredit"><a target="_blank" href="http://crestaproject.com/" title="Premium WordPress Themes"><img src="'. plugins_url( '/images/by-cr-social.png' , __FILE__ ) .'" alt="CrestaProject" /></a></div>';
}
...
<div id="poststuff">
 ...
 <form name="social_buttons" method="post" action="<?php echo str_replace( '%7E', '~', $_SERVER['REQUEST_URI']); ?>">  
		...
		<li class="list-group-item">
			<label><input type="checkbox" id="chkcrestacredit" name="showcreditcresta" <?php if(get_option('cresta_social_shares_show_credit') == "1") { echo 'checked="checked"'; }?> />Show CrestaProject Credit :)</label>
		</li>
		</ul>
		<ul class="list-group">

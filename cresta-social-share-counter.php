...
function cresta_filter_in_content( $content ) { 
	...
	foreach ( $post_types  as $post_type ) { 
		if ( $post_type == get_post_type() && !in_array( $post_type, $show_on )  ) {
			return $content;
		}
	}
...
<div id="poststuff">
 ...
 <form name="social_buttons" method="post" action="<?php echo str_replace( '%7E', '~', $_SERVER['REQUEST_URI']); ?>">  
		...
		<li class="list-group-item">
			<label><input type="checkbox" <?php if(in_array('archive',$show_on)) { echo 'checked="checked"'; }?> name="socialpages[]" value="posts"/>Archives</label>
		</li>
		<?php
			$args = array(
				'public'   => true,
			...
			foreach ( $post_types  as $post_type ) { 
				echo '
					<li class="list-group-item">
						<label><input type="checkbox"';  if(in_array( $post_type ,$show_on)) { echo 'checked="checked"'; } echo' name="socialpages[]" value="' . $post_type . '"/>' . $post_type . ' <i>(Custom Post Type)</i></label>
					</li>
				';
			}
		?>
